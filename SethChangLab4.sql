...
--6B
select supplier_id, count(supplier_id), to_char(sum(price),'$9999.99') as sum_price, to_char(avg(price),'$9999.99')as avg_price
from food
group by supplier_id;

--6C
select supplier_id, count(price_upcharge), to_char(sum(price),'$9999.99') as sum_price, to_char(avg(price),'$9999.99')as avg_price
from food
group by supplier_id;
